        ...
        with open(policy_path, "r") as policy_file:
            ...
            for package in packages:
                ...
                try:
                    with open(license_db_path, "r") as license_db_file:
                        ...
                        if package["package"] in license_db:
                            ...
                            compliance_result["license"] = license_db[
                                package["package"]
                            ]
                            ...
                            if compliance_result["license"] in policy_data["denied"]:
                                ...
                                compliance_result["citation"] = (
                                    f"license '{compliance_result['license']}' matches the 'denied' list in policy.json"
                                )
                            elif compliance_result["license"] in policy_data["warning"]:
                                compliance_result["status"] = "warning"
                                compliance_result["citation"] = (
                                    f"license '{compliance_result['license']}' matches the 'warning' list in policy.json"
                                )
                            elif compliance_result["license"] in policy_data["allowed"]:
                                compliance_result["status"] = "allowed"
                                compliance_result["citation"] = (
                                    f"license '{compliance_result['license']}' matches the 'allowed' list in policy.json"
                                )
                            else:
                                compliance_result["status"] = "unknown"
                                compliance_result["citation"] = (
                                    f"license '{compliance_result['license']}' has unknown status"
                                )
                        else:
                            compliance_result["status"] = "unknown"
                            compliance_result["citation"] = (
                                f"no entry for '{package['package']}' in {license_db_path} — license could not be verified"
                            )
                ...
                # Handle any exceptions that occur during compliance checking
                    ...
                    compliance_result["status"] = "unknown"
                    compliance_result["citation"] = (
                        f"An error occurred while checking package {package['package']}."
                    )
